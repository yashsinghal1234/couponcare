...
requestsRouter.post("/coupons/:couponId/requests", requireAuth, async (req, res) => {
    ...
    if (coupon.status !== "available")
        return res.status(409).json({ error: "Coupon is not available" });
    ...
    if (existing)
        return res.status(409).json({ error: "Already requested", requestId: existing._id.toString(), status: existing.status });
    ...
    if (isAuto) {
        const updated = await CouponModel.findOneAndUpdate({ _id: coupon._id, status: "available", claimedByRequestId: { $exists: false } }, { $set: { status: "claimed", claimedByRequestId: request._id } }, { new: true });
        if (!updated) {
...
requestsRouter.post("/requests/:id/approve", requireAuth, async (req, res) => {
    ...
    if (!request)
        return res.status(404).json({ error: "Not found" });
    if (request.donorId.toString() !== donorId)
        return res.status(403).json({ error: "Forbidden" });
    const coupon = await CouponModel.findById(request.couponId);
    if (!coupon)
        return res.status(404).json({ error: "Coupon missing" });
    if (coupon.revealMode !== "donorApproval")
        return res.status(409).json({ error: "Approval not required for this coupon" });
    if (coupon.status !== "available")
        return res.status(409).json({ error: "Coupon not available" });
    if (request.status !== "pending")
        return res.status(409).json({ error: "Request not pending" });
    const updatedCoupon = await CouponModel.findOneAndUpdate({ _id: coupon._id, status: "available", claimedByRequestId: { $exists: false } }, { $set: { status: "claimed", claimedByRequestId: request._id } }, { new: true });
    if (!updatedCoupon)
        return res.status(409).json({ error: "Coupon already claimed" });
