 ...
 * Includes reward/cashback coupon detection
 */
...
export type CouponSource = "RegularPromo" | "HDFCHupi" | "PaytmReward" | "UPIReward" | "DirectBrandReward" | "CreditCardReward" | "Other";
...
export interface DetectionResult {
    ...
    isReward?: boolean; // true if detected as reward/cashback coupon
...
const BRAND_PATTERNS: Record<string, BrandPattern> = {
    ...
    // REWARD & CASHBACK COUPONS (HDFC, Paytm, Direct Brand Rewards)
    "Lenskart": {
        patterns: [/lenskart|eyeglasses|spectacles|contact lens|₹500|₹1000|₹2000|vision|lens/i],
        category: "Shopping",
        type: "FlatAmount",
        transferable: true,
        suggestedDescription: "Lenskart reward or cashback coupon",
    },
    "Mobillera": {
        patterns: [/mobillera|mobile|phone|cashback|reward|₹500|₹1000/i],
        category: "Shopping",
        type: "Cashback",
        transferable: true,
        suggestedDescription: "Mobillera mobile reward coupon",
    },
    "HDFC Hupi": {
        patterns: [/hdfc|hupi|hdfc hupi|credit card|reward|cashback|₹|HDF-/i],
        category: "Payment",
        type: "Cashback",
        transferable: false,
        warning: "⚠ HDFC Hupi codes may be account-specific. Verify transferability before donating.",
    },
    "Myntra Fashion": {
        patterns: [/myntra rewards?|myntra gift|myntra cashback|myntra credit|M-|fashion voucher/i],
        category: "Shopping",
        type: "FlatAmount",
        transferable: true,
        suggestedDescription: "Myntra reward/cashback coupon",
    },
    "Amazon Voucher": {
        patterns: [/amazon voucher|amazon gift|amazon balance|amazon credit|ANZ-|AMZ-/i],
        category: "Shopping",
        type: "FlatAmount",
        transferable: true,
        suggestedDescription: "Amazon gift voucher or reward",
    },
    "Flipkart Gift Card": {
        patterns: [/flipkart gift|flipkart voucher|flipkart credit|flipkart reward|FK-/i],
        category: "Shopping",
        type: "FlatAmount",
        transferable: true,
        suggestedDescription: "Flipkart gift card or reward",
    },
};

export function detectRewardSource(input: string): CouponSource {
    const lowerInput = (input || "").toLowerCase();

    if (/hdfc|hupi/i.test(lowerInput)) return "HDFCHupi";
    if (/paytm|ptm/i.test(lowerInput)) return "PaytmReward";
    if (/upi|gpay|google pay|phonepe|bhim/i.test(lowerInput)) return "UPIReward";
    if (/credit card|cc|card|reward|points/i.test(lowerInput)) return "CreditCardReward";
    if (/brand reward|direct|store credit|voucher/i.test(lowerInput)) return "DirectBrandReward";

    return "Other";
}

export function isRewardCoupon(input: string, brand: string): boolean {
    const lowerInput = (input || "").toLowerCase();
    ...
    // Check if input mentions reward or if brand is reward-associated
    return rewardKeywords.test(lowerInput) && rewardBrands.test(lowerInput + lowerBrand);
}
...
export function detectCoupon(input: string): DetectionResult {
    ...
    // Detect if this is a reward coupon
    const isReward = isRewardCoupon(trimmed, brand);
    const rewardSource = detectRewardSource(trimmed);
